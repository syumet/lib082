...
class KnapsackInstance {
	...
	public Double value(int i) {return this.items[i].getValue();}
	
	/** Obtain and item's weight */
	public Double weight(int i) {return this.items[i].getWeight();}

	/** Obtain the knapsack's capacity */
	public Double capacity() {return this.W;}
	
	/** Obtain the array of items. */
	public KnapsackItem[] items() {return this.items;}
	...
	public String toString() {
		...
		for(int i=0; i < this.numItems; i++) {
			result.append(this.items[i].getValue() + ", " + this.items[i].getWeight() + "\n");
		}
...
//		sortByDens(K.items());
	...
	/**
	 * @precond K.items has been sorted in descending order 
	 * @param K an instance of knapsack
...
//		return stochasticGreedy(K, 0, 0, 0, 0);
	...
	static double stochasticGreedy
		(KnapsackInstance K, double bv, double cv, double cw, int i) {
		if (i >= K.numItems()) return cv;
		if (cw + K.weight(i) <= K.capacity())
			bv = stochasticGreedy(K, bv, cv + K.value(i), cw + K.weight(i), i+1);
		else
			bv = stochasticGreedy(K, bv, cv, cw, i+1);
		// TODO improve acceptance probability function, apparently works good with items <= 50 
	...
	public static void main(String args[]) {
		KnapsackInstance K = readKnapsackInstance("Knapsack-Template/knapsack-1000.dat");

...
public class OptimalBST {
	...
	/** 
	 * The upper triangular part (including diagonal entries) of the cost matrix is used to record 
	 * the cost of the optimal tree, i.e. cost[a][b] = minimum cost starting from a to b.
	 * The lower triangular part of the cost matrix is used to record the root of the optimal tree.
	 * i.e. cost[b][a] = position of root of the tree starting from a to b.
	 * */
	final double cost[][];
	...
	private void calCost() {
		...
		for (int i = 1; i < n; i++) {
			...
			for (int j = 0; j + i < n; j++) {
				...
				deepCost += cost[k][k];				
				cost[j][j+i] = minSubCost + deepCost; // update the cost
				cost[j+i][j] = root; // update the root 
			}
		}
	}
	...
	public String toString() {	
		...
		for (int i = 0; i < n; i++) {
			for (int j = i + 1; j < n; j++) {
				double c = cost[i][j];				
				s.append(Double.valueOf(df.format(c)) + "|" + words[(int) cost[j][i]] + ", ");	
			}
			s.append("\n");
		}
		return s.toString();

...
public class Morphology<E> {
  ...
  public int numOfComponents(double[][] I, Connectivity connectivity)
  ...
  private UnionFind<E> runUnionFind(double[][] I, Connectivity connectivity)
      throws IllegalArgumentException {
    ...
    UnionFind<E> uf = new UnionFind<>(rows * cols);
    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
        if (I[i][j] > 0) {
          for (int[] d : distance) {
